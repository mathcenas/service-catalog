...
$logFiles = Get-ChildItem -LiteralPath $KLS_LOG_DIR -ErrorAction SilentlyContinue |
    Where-Object { $_.Extension -in @('.log', '.txt') -and $_.LastWriteTime -gt $since } |
    Sort-Object LastWriteTime -Descending

if ($KLS_JOB_NAME) {
    $logFiles = $logFiles | Where-Object {
        (Get-Content -LiteralPath $_.FullName -TotalCount 5 -ErrorAction SilentlyContinue) -match [regex]::Escape($KLS_JOB_NAME)
    }
}
...
$content = Get-Content -LiteralPath $logFile.FullName -Encoding UTF8 -ErrorAction Stop
