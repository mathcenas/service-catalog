...
export function TelemetryDashboard({ services, clients }: Props) {
  ...
  const serviceCards = useMemo(() => {
    ...
    const withHeartbeat = Array.from(serviceIds).map(serviceId => {
      const svc = services.find(s => s.id === serviceId && s.telemetry_enabled === true);
      const client = svc ? clients.find(c => c.id === svc.client_id) : null;
    ...
    const silent = services
      .filter(s => !serviceIds.has(s.id) && s.status === 'Active' && s.telemetry_enabled === true)
      .map(svc => {
        ...
        <div className="space-y-4">
          <div className="flex items-start justify-between flex-wrap gap-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h2 className="text-xl font-bold text-gray-900">Accesos SMB</h2>
...
function loadSoftwareForClient(clientId?: string): SoftwareImport[] {
  if (!clientId) return [];
  try {
    const all = JSON.parse(localStorage.getItem('software_inventory_imports') || '[]') as SoftwareImport[];
    return all.filter(f => f.clientId === clientId);
  } catch { return []; }
}

function buildOwnerMapFromImports(files: SoftwareImport[]): Map<string, string> {
  const map = new Map<string, string>();
  files.filter(f => f.type === 'entra').flatMap(f => f.entraDevices ?? [])
