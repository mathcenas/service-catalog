        ...
        <div style="font-family:${EMAIL_FONT};max-width:600px;margin:0 auto;padding:32px 24px;background:#f8fafc;">
          <div style="background:#ffffff;border-radius:12px;padding:28px;border:1px solid ${B.border};">
            ${emailHeader({
              ...
              label: "Revisión de usuarios SMB",
              accentColor: "#7c3aed",
              title: "Acción requerida",
              subtitle: `${client?.company_name || ""} — ${svc?.business_name || svc?.name || "NAS"}`,
            })}
            <p style="color:${B.textMain};font-size:14px;margin:0 0 16px;">El cliente completó la revisión de usuarios del servidor NAS${generatedDate ? ` (reporte del ${generatedDate})` : ""}.</p>
            <table style="width:100%;border-collapse:collapse;margin:0 0 16px;border:1px solid ${B.border};border-radius:8px;overflow:hidden;">
              <thead>
                <tr style="background:${B.bg};">
                  <th style="padding:10px 12px;text-align:left;font-size:11px;color:${B.textMid};font-weight:600;text-transform:uppercase;letter-spacing:.4px;">Usuario</th>
                  <th style="padding:10px 12px;text-align:left;font-size:11px;color:${B.textMid};font-weight:600;text-transform:uppercase;letter-spacing:.4px;">Acción</th>
                  <th style="padding:10px 12px;text-align:left;font-size:11px;color:${B.textMid};font-weight:600;text-transform:uppercase;letter-spacing:.4px;">Nota</th>
                </tr>
              </thead>
            ...
            ${emailPortalPanel({ companyName, portalUrl })}
